...
const TargetCursor: React.FC<TargetCursorProps> = ({
    ...
    useEffect(() => {
        ...
        // Separate check function for re-evaluating target under mouse
        const checkTargetUnderMouse = () => {
        ...
        const tickerFn = () => {
            ...
            // Periodic check if static mouse is still over the correct target
            // Especially important for swapping/moving elements like in CardSwap
            if (activeTarget && (!activeTarget.isConnected || gsap.ticker.frame % 20 === 0)) {
                const currentTopTarget = checkTargetUnderMouse();
                if (currentTopTarget !== activeTarget) {
                    currentLeaveHandler?.();
                    if (currentTopTarget) {
                        enterHandler({ target: currentTopTarget } as any);
                    }
                    return;
                }
            }
            ...
            corners.forEach((corner, i) => {
                ...
                // Optimized smoothing: 
                // When transitioning (strength < 1), use lerp. 
                // When fully active, follow more tightly with minimal lag.
                const finalX = currentX + (targetX - currentX) * strength;
                const finalY = currentY + (targetY - currentY) * strength;

                const duration = strength >= 0.99 ? (parallaxOn ? 0.08 : 0) : 0.1;
        ...
        const scrollHandler = () => {
            ...
            const isStillOverTarget = checkTargetUnderMouse() === activeTarget;
